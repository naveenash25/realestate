...
export class NotificationsService {
  ...
  async sendLeadNotification(payload: { enquiry: any; owner_id: string; buyer_name: string; buyer_phone: string }) {
    ...
    await Promise.allSettled([
      ...
      this.sendWhatsApp(payload.owner_id, payload.buyer_name, payload.buyer_phone),
    ]);
  }
  ...
  private async sendWhatsApp(ownerId: string, buyerName: string, buyerPhone: string) {
    try {
      await axios.post(
        `https://graph.facebook.com/v18.0/${process.env.WHATSAPP_PHONE_NUMBER_ID}/messages`,
        {
          ...
          template: {
            ...
            language: { code: 'en' },
            components: [{ type: 'body', parameters: [{ type: 'text', text: buyerName }, { type: 'text', text: buyerPhone }] }],
          },
        },
