...
export class PropertiesService {
  ...
  async search(query: SearchPropertyDto) {
    ...
    if (query.location_tier) {
      conditions.push(`p.location_tier = $${i++}`);
      params.push(query.location_tier);
    }
    if (query.property_type) {
      conditions.push(`p.property_type = $${i++}`);
      params.push(query.property_type);
    }
    if (query.rera_status) {
      conditions.push(`p.rera_status = $${i++}`);
      params.push(query.rera_status);
    }
    if (query.min_price) {
      conditions.push(`p.price_value >= $${i++}`);
  ...
  async update(id: string, userId: string, dto: Partial<CreatePropertyDto>) {
    ...
    if (!property[0])
      throw new ForbiddenException('Property not found or not yours');
  ...
  async remove(id: string, userId: string) {
    await this.dataSource.query(
      `UPDATE properties SET is_active = false WHERE id = $1 AND owner_id = $2`,
      [id, userId],
    );
    return { success: true };
  }
  ...
  async upsertDocument(
    ...
    ownerId: string,
    type: string,
    doc: { path: string; originalName: string; mimeType: string; size: number },
  ) {
    await this.assertOwnership(propertyId, ownerId);
    const result = await this.dataSource.query(
      `INSERT INTO property_documents (property_id, type, file_path, original_name, mime_type, size_bytes)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (property_id, type) DO UPDATE
         SET file_path = EXCLUDED.file_path, original_name = EXCLUDED.original_name,
             mime_type = EXCLUDED.mime_type, size_bytes = EXCLUDED.size_bytes,
             status = 'pending', verified_at = NULL, verified_by = NULL
       RETURNING *`,
      [propertyId, type, doc.path, doc.originalName, doc.mimeType, doc.size],
    );
    return result[0];
  }

  async listDocuments(propertyId: string, ownerId: string) {
    await this.assertOwnership(propertyId, ownerId);
    return this.dataSource.query(
      `SELECT * FROM property_documents WHERE property_id = $1 ORDER BY created_at`,
      [propertyId],
    );
  }

  async removeDocument(propertyId: string, ownerId: string, type: string) {
    await this.assertOwnership(propertyId, ownerId);
    await this.dataSource.query(
      `DELETE FROM property_documents WHERE property_id = $1 AND type = $2`,
      [propertyId, type],
    );
    return { success: true };
  }
